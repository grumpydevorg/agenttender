...
- Optional `tender-screen` Rust extension: screen reads, waits, and checkpoint-based reattach through a versioned external CLI contract; no Ghostty/Zig dependency in core
...
- Terminal emulation/rendering — optional external `tender-screen` or Boo. Revision of the 2026-07-09 decision: core may dispatch screen commands to an explicitly installed extension, but does not link or embed a VT engine.
