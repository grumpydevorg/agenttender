...
links:
  ...
  - 2026-09-25-dagger-ci-data/README.md
---

# Dagger CI pilot

**Complete, not adopted (2026-09-25).** Dagger did not become part of this
repository's CI. The module, sandbox script and workflow the pilot used are in
closed PR #78 (head `2e9a66c`), not in the tree. The only CI change that
shipped is the baseline Windows cross-target clippy job (#77). Original
timing samples are in [2026-09-25-dagger-ci-data/](2026-09-25-dagger-ci-data/README.md).
...
adopted) with sources under `.dagger/` (both in #78). Go is the most complete SDK; its
...
- The CLI runs in a container with `--network none` and only the Docker
  socket mounted (`.github/dagger-sandboxed.sh` in #78, used by CI and locally
  alike).
...
from cache. Failed samples are kept in the timings above. Tracked in #80, and
in #76 for the harness deadline test.
...
option for this public repository has a security or cost trade-off (table
below).
...
Linux `dagger` binary and a privileged engine container. The gain alone did
not justify keeping a second definition of the checks beside `ci.yml` (see
Decisions).
...
## Decisions (2026-09-25, owner)

- **Dagger CI not adopted:** #78 closed without merging. The module is not kept
  for local use.
- **Findings and original samples** landed as this documentation-only change.
- **The baseline stays:** #75, then #77, and the new `clippy windows targets
  (ubuntu)` job becomes a required check on `main`, with the existing required
  checks preserved.
- **The four intermittent tests** are tracked separately: #80 covers three,
  and #76 covers the harness deadline test.
- **The persistent-engine experiment moves to edge-platform.** Its private
  repository and existing self-hosted runners make a long-lived engine
  acceptable there. It is recorded in that repository's CI/CD plan
  (`prototypes/edge-platform/docs/plans/backlog/build-pipeline/edge_platform_cicd.md`,
  "Persistent-engine experiment").
- **Cloud Checks** was not tested and is not pursued. It would need the Team
  plan and trace upload.

Persistent-engine options considered for this repository, none set up:
...
Original samples, one CSV per measurement set, described in
[2026-09-25-dagger-ci-data/README.md](2026-09-25-dagger-ci-data/README.md).
The per-run logs were not kept. GitHub keeps the hosted runs' logs under the
run IDs listed there.
